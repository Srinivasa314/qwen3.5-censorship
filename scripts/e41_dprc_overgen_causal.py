"""Causal test of d_prc overgeneralization.

Steer d_prc at L13 with α in {0, -5, -10, -15, -20} on the three flagged
non-PRC sovereignty/regime-criticism prompts (Kosovo, Catalonia, Saudi).
At α=0 these emit a PRC-style template; the sweep measures the α at which
subtracting d_prc flips each back to factual non-PRC framing.
"""
...
def main():
    ...
    ap.add_argument("--alphas", nargs="+", type=float, default=[0.0, -5.0, -10.0, -15.0, -20.0])
    ap.add_argument("--out", default=str(config.RESULTS_DIR / "e41_dprc_overgen_causal.json"))
    args = ap.parse_args()

    meta = data.load()["_meta"]["overgeneralization_and_anomalies"]
    flagged_ids = [e["id"] for e in meta["dprc_overgeneralization"]]
    by_id = data.by_id()
    items = [by_id[i] for i in flagged_ids]
    prompts = [it["chat"] for it in items]
    print(f"d_prc overgen IDs: {flagged_ids}", flush=True)
    ...
    for alpha in args.alphas:
        specs = () if alpha == 0.0 else \
            ((lm.layers[13], "forward", make_steer_hook(d_unit, alpha)),)
